                ...
                >
                                ...
                                <DropdownMenu.Item
                                        ...
                                        on:click={async () => {
                                                if (fileUploadEnabled) {
                                                        // Use Capacitor Camera plugin instead of hidden input for mobile
                                                        if (!Capacitor.isNativePlatform()) { // Check if running in Capacitor
                                                                // Use Camera plugin on native platforms
                                                                try {
                                                                        const image = await Camera.getPhoto({
                                                                                quality: 90,
                                                                                allowEditing: false, // Optional: set true to allow cropping/editing
                                                                                resultType: CameraResultType.Uri, // Get file URI, easier to handle
                                                                                source: CameraSource.Prompt // Ask user: Camera or Gallery
                                                                        });

                                                                        // Convert URI to Blob/File and pass to handler
                                                                        if (image.webPath) {
                                                                                const response = await fetch(image.webPath);
                                                                                const blob = await response.blob();
                                                                                const fileName = image.webPath.split('/').pop() || `capture.${image.format}`;
                                                                                const file = new File([blob], fileName, { type: blob.type });
                                                                                inputFilesHandler([file]); // Pass as array
                                                                        }
                                                                } catch (e) {
                                                                ...
                                                                // Fallback for non-Capacitor environments (e.g., web browser)
                                                                // or keep original screen capture logic if desired
                                                                screenCaptureHandler(); // Or trigger a standard file input
                                                        }
                                                }
                                        }}
                                >
                        ...
                        >
                                <DropdownMenu.Item
                                        ...
                                        on:click={async () => {
                                                if (fileUploadEnabled) {
                                                        // Use Capacitor Camera plugin instead of standard file input
                                                        if (!Capacitor.isNativePlatform()) { // Check if running in Capacitor
                                                                try {
                                                                        // Allow multiple selections if desired, but getPhoto gets one at a time
                                                                        // For multiple, consider Camera.pickImages() or loop getPhoto
                                                                        const image = await Camera.getPhoto({
                                                                                quality: 90,
                                                                                allowEditing: false,
                                                                                resultType: CameraResultType.Uri,
                                                                                source: CameraSource.Photos // Prioritize Gallery for "Upload"
                                                                        });

                                                                        if (image.webPath) {
                                                                                const response = await fetch(image.webPath);
                                                                                const blob = await response.blob();
                                                                                const fileName = image.webPath.split('/').pop() || `upload.${image.format}`;
                                                                                const file = new File([blob], fileName, { type: blob.type });
                                                                                inputFilesHandler([file]); // Pass as array
                                                                        }
                                                                } catch (e) {
                                                                ...
                                                                uploadFilesHandler(); // Keep original handler for web
                                                        }
                                                }
                                        }}
                                >
